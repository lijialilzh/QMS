...
- 打开/刷新时**仅 2.3** 会把缺失的模板子节补回。2.6 算法和数据要求等其它章节删除后写入文档树，刷新不再从标准模板恢复。

### 6.8 标准/其他需求表只挂在 2.1
- `srs_reqs` / `srs_reqs_2` **不作为左目录独立章节**（不再出现「产品功能列表如下：」这类无编号项）。
- 点 2.1 软件总体描述时，把同一「2 软件整体架构要求」下的需求表节点内联到正文；不要求紧挨在 2.1 后面（排到 2.7 之后也能挂上）。
- 内联的需求表节点不显示「请输入标题」；表头「产品功能列表如下：」仍用节点 `label`。
- 若当前选中的是隐藏的需求表节点，编辑区自动落到 2.1。
- 第 7 章功能描述自动生成、其他需求对应 2.5/2.7 等章节的同步不改。

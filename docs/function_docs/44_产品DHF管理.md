...
1. **列表页** `/prod_dhfs`：按**产品**一行展示（名称、完整版本、发布版本、型号、DHF 条数）；模糊搜索产品；行内 **查看 / 编辑 / 复制 / 删除** 进入或操作该产品 DHF；**删除**清空该产品全部 DHF 条目（无条目时禁用）；**复制**仅可复制到**尚无 DHF** 的目标产品版本（可同产品名称不同完整版本；禁止源相同版本、禁止目标已有 DHF）；顶部 **新增** 选产品后进入编辑页。

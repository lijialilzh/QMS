...
export default () => {
    ...
    const load = () => {
        setLoading(true);
        ApiIntegrate.list_export_records({ page_index: 0, page_size: 100 }).then((res: any) => {
            setLoading(false);
    ...
    const columns = [
        ...
        {
            title: "导出文件", dataIndex: "doc_names", ellipsis: true,
            render: (v: string) => v ? (
                <Tooltip title={v}>
        ...
        { title: "导出时间", dataIndex: "create_time" },
        { title: "文件名", dataIndex: "filename", render: (v: string) => <span style={{ fontSize: 12, color: "#6b7280" }}>{v}</span> },
    ];

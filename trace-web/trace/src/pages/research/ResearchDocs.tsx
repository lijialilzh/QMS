...
export default () => {
    ...
    useEffect(() => {
        ...
        Api.list_research_doc({ product_id: productId, page_index: 0, page_size: 10000 }).then((res: any) => {
    ...
    const doAdd = () => {
        addForm.validateFields().then((values) => {
            ...
            Api.add_research_doc({ ...values }).then((res: any) => {
                ...
                if (res.code === Api.C_OK) {
                    ...
                    if (newId) {
                        navigate(`/research_docs/edit/${newId}`);
                    } else {
    ...
    const doDelete = () => {
        dispatch({ loading: true });
        Api.delete_research_doc({ id: data.targetRow.id }).then((res: any) => {
            if (res.code === Api.C_OK) {
                message.success(ts("save_success"));
    ...
    const doBatchDelete = createDocBatchDelete({
        ...
        dispatch,
        data,
        deleteFn: Api.delete_research_doc,
        cOk: Api.C_OK,
    ...
    const doCopy = () => {
        ...
        if (!row.id) return;
        dispatch({ loading: true });
        Api.duplicate_research_doc({ id: row.id, product_id: data.copyProductId }).then((res: any) => {
            dispatch({ loading: false });
            if (res.code === Api.C_OK) {
                dispatch({ dlgType: null });
    ...
    const doExport = async (row: any) => {
        ...
        try {
            const res: any = await Api.export_research_doc({ id: row.id });
            if (res.code !== Api.C_OK) {
        ...
        { title: ts("create_time"), dataIndex: "create_time", width: "14%" },
        {
            ...
            render: (_: any, row: any) => (
                <Space size={4} className="risk-doc-action-space">
                    <Button type="link" size="small" onClick={() => navigate(`/research_docs/view/${row.id}`)}>
                        {ts("view")}
                    </Button>
                    <Button type="link" size="small" onClick={() => navigate(`/research_docs/edit/${row.id}`)}>
                        {ts("edit")}
                    </Button>

...
// 从时间线里找「软件测试计划」文件所在行的日期（取最早匹配行），格式「YYYY年M月D日」
const computeFileDate = (rows: any[], keyword = "软件测试计划"): string => {
    const num = (v: any) => parseInt(String(v ?? "").replace(/[^\d]/g, ""), 10);
...
export default () => {
        ...
        new Promise((resolve) => {
            ...
            Promise.all([
                ...
                out = fillMilestone(out, {
                    ...
                    planDate: latestDateFor(tlRows, ["软件测试计划"]),
    ...
    const load = () => {
        ...
        Api.get_stp_doc({ id }).then((res: any) => {
    ...
    const doSave = () => {
        ...
        Api.update_stp_doc({ id, content, product_id: data.doc.product_id, version: data.doc.version }).then((res: any) => {
    ...
    const doExport = async () => {
        ...
        try {
            const res: any = await Api.export_stp_doc({ id });
        ...
        <div className="div-v page pdp-detail">
            <div className="div-h pdp-toolbar">
                ...
                <Space>
                    ...
                    <Button onClick={() => navigate("/stp_docs")}>{ts("back")}</Button>
                </Space>
            </div>

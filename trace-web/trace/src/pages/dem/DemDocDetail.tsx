...
export default () => {
    ...
    const load = () => {
        ...
        Api.get_dem_doc({ id }).then((res: any) => {
    ...
    const doSave = () => {
        ...
        Api.update_dem_doc({ id, content: data.content, product_id: data.doc.product_id, version: data.doc.version }).then((res: any) => {
    ...
    const doExport = async () => {
        ...
        try {
            const res: any = await Api.export_dem_doc({ id });
        ...
        <div className="div-v page pdp-detail">
            <div className="div-h pdp-toolbar">
                ...
                <Space>
                    ...
                    <Button onClick={() => navigate("/dem_docs")}>{ts("back")}</Button>
                </Space>
            </div>
            ...
            <Spin spinning={data.loading} wrapperClassName="pdp-scroll">
                <div style={{ height: "100%", overflow: "auto" }}>
                    <div style={{ padding: "12px 20px" }}>
                        <div style={{ textAlign: "center", fontSize: 16, fontWeight: 700, margin: "4px 0 12px" }}>开发环境维护说明</div>
                        <div style={{ color: "#888", marginBottom: 4 }}>{data.doc.file_no || ""}</div>

                        {/* 资产表 */}
                        <div style={{ color: "#888", margin: "16px 0 4px" }}>开发环境维护记录 · 资产</div>
                                ...
                                <div key={ci} style={{ marginTop: 16, overflowX: "auto" }}>
                                    <table style={{ ...tableStyle, minWidth: 1100, tableLayout: "auto" }}>
                                        <tbody>
                                            ...
                                            {rows.length === 0 ? (
                                                <tr><td colSpan={ncols} style={{ ...tdValue, textAlign: "center", color: "#bbb" }}>该产品未查询到「开发~测试」时间线，暂无周记录</td></tr>
                                            ) : rows.map((row, ri) => {
                                                let checkIdx = -1;

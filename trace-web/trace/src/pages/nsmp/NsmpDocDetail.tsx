...
import * as Api from "@/api/ApiNsmpDoc";
...
// 编号：封面/修订记录不编号；其余正文顶级 1/2/3，子级 1.1...
const NO_NUM = new Set(["cover", "revision"]);
...
export default () => {
    ...
    const load = () => {
        ...
        Api.get_nsmp_doc({ id }).then((res: any) => {
    ...
    const rebindProduct = (newId: number) => {
        ...
        Api.nsmp_autofill({ product_id: newId, version: data.doc.version || "" }).then((res: any) => {
            if (res.code !== Api.C_OK) {
                dispatch({ loading: false });
                message.error(res.msg);
                return;
    ...
    const doSave = () => {
        ...
        Api.update_nsmp_doc({ id, content, product_id: data.doc.product_id, version: data.doc.version }).then((res: any) => {
    ...
    const doExport = async () => {
        ...
        try {
            const res: any = await Api.export_nsmp_doc({ id });
    ...
    const isAuto = active && (active.ref_type === "maint_scope");
        ...
        <div className="div-v page pdp-detail">
            <div className="div-h pdp-toolbar">
                ...
                <Space>
                    ...
                    <Button onClick={() => navigate("/nsmp_docs")}>{ts("back")}</Button>
                </Space>
            </div>
            ...
            <Spin spinning={data.loading} wrapperClassName="pdp-scroll">
                <div className="pdp-layout">
                    <div className="pdp-nav">
                        ...
                        {!readonly && (
                            <div className="pdp-nav-hint">点章节改名/编辑，右侧 + 加子章节、🗑 删除；编号按层级自动生成（封面/修订记录不编号）。产品名称/版本自动填入「维护范围」。</div>
                        )}
                            ...
                            <>
                                <div className="pdp-field">
                                    ...
                                    <Input
                                        ...
                                        disabled={readonly}
                                        placeholder="只填名称，如：维护范围"
                                        onChange={(e) => patchNode(active._key, { title: e.target.value })}
                                    />
                                </div>
                                <div className="pdp-field">
                                    <div className="pdp-label">正文{isAuto ? "（产品名称/版本自动获取）" : ""}</div>
                                    <Input.TextArea
                                ...
                                {active.body_after !== undefined && (
                                    <div className="pdp-field">
                                        <div className="pdp-label">补充说明（表格后正文）</div>
                                        <Input.TextArea
                                            autoSize={{ minRows: 2, maxRows: 12 }}
                                            value={active.body_after ?? ""}
                                            disabled={readonly}
                                            onChange={(e) => patchNode(active._key, { body_after: e.target.value })}
                                        />
                                    </div>
                                )}

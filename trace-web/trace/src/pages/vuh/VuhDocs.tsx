...
export default () => {
    ...
    useEffect(() => {
        ...
        Api.list_vuh_doc({ product_id: productId, page_index: 0, page_size: 10000 }).then((res: any) => {
    ...
    const doSearch = (params: any = {}, pageIndex: any = data.pageIndex, pageSize: any = data.pageSize) => {
        ...
        Api.list_vuh_doc({ ...sanitizeListQuery(params), page_index: pageIndex - 1, page_size: pageSize }).then((res: any) => {
            if (res.code === Api.C_OK) {
    ...
    const doAdd = () => {
        addForm.validateFields().then((values) => {
            ...
            Api.add_vuh_doc({ ...values }).then((res: any) => {
    ...
    const doDelete = () => {
        dispatch({ loading: true });
        Api.delete_vuh_doc({ id: data.targetRow.id }).then((res: any) => {
            if (res.code === Api.C_OK) {
                message.success(ts("save_success"));
    ...
    const doCopy = () => {
        ...
        if (!row.id) return;
        dispatch({ loading: true });
        Api.duplicate_vuh_doc({ id: row.id, product_id: data.copyProductId }).then((res: any) => {
            dispatch({ loading: false });
            if (res.code === Api.C_OK) {
                dispatch({ dlgType: null });
    ...
    const doExport = async (row: any) => {
        ...
        try {
            const res: any = await Api.export_vuh_doc({ id: row.id });
            if (res.code !== Api.C_OK) {
    ...
    const handleSaveFileNo = async (row: any) => {
        ...
        try {
            const res: any = await Api.update_vuh_doc({ id: row.id, file_no: nextFileNo });
            if (res.code === Api.C_OK) {
        ...
        { title: ts("create_time"), dataIndex: "create_time", width: "14%" },
        {
            ...
            render: (_: any, row: any) => (
                <Space size={4} className="risk-doc-action-space">
                    <Button type="link" size="small" onClick={() => navigate(`/vuh_docs/view/${row.id}`)}>
                        {ts("view")}
                    </Button>
                    <Button type="link" size="small" onClick={() => navigate(`/vuh_docs/edit/${row.id}`)}>
                        {ts("edit")}
                    </Button>

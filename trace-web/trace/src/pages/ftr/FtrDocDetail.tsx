...
// 从时间线里找「现场测试规程」文件所在行的日期（取最早匹配行），格式「YYYY年M月D日」
const computeFileDate = (rows: any[], keyword = "现场测试规程"): string => {
    const num = (v: any) => parseInt(String(v ?? "").replace(/[^\d]/g, ""), 10);
...
export default () => {
        ...
        new Promise((resolve) => {
            ...
            Promise.all([
                ...
                out = fillMilestone(out, {
                    ...
                    planDate: latestDateFor(tlRows, ["现场测试规程"]),
    ...
    const load = () => {
        ...
        Api.get_ftr_doc({ id }).then((res: any) => {
    ...
    const doSave = () => {
        ...
        Api.update_ftr_doc({ id, content, product_id: data.doc.product_id, version: data.doc.version }).then((res: any) => {
    ...
    const doExport = async () => {
        ...
        try {
            const res: any = await Api.export_ftr_doc({ id });
        ...
        <div className="div-v page pdp-detail">
            <div className="div-h pdp-toolbar">
                ...
                <Space>
                    ...
                    <Button onClick={() => navigate("/ftr_docs")}>{ts("back")}</Button>
                </Space>
            </div>

...
const renameInspectChapter = (nodes: any[], docType: string): any[] => {
    const oldName = inspectOldTitle(docType);
    const newName = inspectTitle(docType);
    return (nodes || []).map((n: any) => ({
        ...n,
        title: stripNum(n.title) === oldName ? newName : n.title,
        children: renameInspectChapter(n.children || [], docType),
    }));
};

const ensureEnvMaintChapter = (nodes: any[], docType: string): any[] => {
    const renamed = renameInspectChapter(nodes, docType);
    const want = maintTitle(docType);
    const after = inspectTitle(docType);
    const withHeader = mapByTitle(renamed, after, (n: any) => {
        const tables = (n.tables || []).map((tb: any[]) => {
            if (!Array.isArray(tb) || !Array.isArray(tb[0])) return tb;
            const hdr = tb[0].map((h: any) => String(h || ""));
            if (String(hdr[0] || "").trim() === "资产编码") return tb;
            return hdr.length >= 2 ? [["资产编码", "设备信息", "产品名称", "完整版本"], ...tb] : tb;
        });
        return { ...n, tables };
    });
    if (findByTitle(withHeader, want)) return withHeader;
    const node = { title: want, body: "", tables: [], children: [] };
    const idx = (withHeader || []).findIndex((n: any) => stripNum(n.title) === after);
    if (idx >= 0) {
        const next = [...withHeader];
        next.splice(idx + 1, 0, node);
        return next;
    }
    return [...(withHeader || []), node];
};
...
export default () => {
    ...
    const docType = type === "dd_017" ? "dd_017" : "dd_016";
    const title = getDataDocMeta(docType).title;
    ...
    const applyEnv = (productId: number, secs: any[]): Promise<any[]> =>
        new Promise((resolve) => {
            ...
            Promise.all([
                ...
                const checkerName = memberNames(members, (r) => r === "数据部负责人")[0]
                    || memberNames(members, (r) => r === "数据负责人")[0]
    ...
    const assetLabel = docType === "dd_017" ? "标注环境维护记录 · 资产" : "开发环境维护记录 · 资产";
                    ...
                    <div style={{ padding: "12px 20px" }}>
                        ...
                        <div style={{ border: "1px solid #eee", background: "#fafafa", borderRadius: 4, padding: "10px 12px", lineHeight: 1.7, color: "#333", fontSize: 13 }}>
                            {desc.split("\n").map((ln: string, i: number) => {
                                const t = ln.trim();
                                const isHead = t === "开发/测试环境定期验证" || t.includes("检查项为") || t.includes("验证项为") || /^[一二三四五六七八九十]：/.test(t);

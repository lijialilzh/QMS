...
export default () => {
    ...
    useEffect(() => {
        ...
        Api.list_cybersec_plan_doc({ product_id: productId, page_index: 0, page_size: 10000 }).then((res: any) => {
    ...
    const doSearch = (params: any = {}, pageIndex: any = data.pageIndex, pageSize: any = data.pageSize) => {
        ...
        Api.list_cybersec_plan_doc({ ...sanitizeListQuery(params), page_index: pageIndex - 1, page_size: pageSize }).then((res: any) => {
            if (res.code === Api.C_OK) {
    ...
    const doAdd = () => {
        addForm.validateFields().then((values) => {
            ...
            Api.add_cybersec_plan_doc({ ...values }).then((res: any) => {
                ...
                if (res.code === Api.C_OK) {
                    ...
                    if (newId) {
                        navigate(`/cybersec_plan_docs/edit/${newId}`);
                    } else {
    ...
    const doDelete = () => {
        dispatch({ loading: true });
        Api.delete_cybersec_plan_doc({ id: data.targetRow.id }).then((res: any) => {
            if (res.code === Api.C_OK) {
                message.success(ts("save_success"));
    ...
    const doCopy = () => {
        ...
        if (!row.id) return;
        dispatch({ loading: true });
        Api.duplicate_cybersec_plan_doc({ id: row.id, product_id: data.copyProductId }).then((res: any) => {
            dispatch({ loading: false });
            if (res.code === Api.C_OK) {
                dispatch({ dlgType: null });
    ...
    const doExport = async (row: any) => {
        ...
        try {
            const res: any = await Api.export_cybersec_plan_doc({ id: row.id });
            if (res.code !== Api.C_OK) {
    ...
    const handleSaveFileNo = async (row: any) => {
        ...
        try {
            const res: any = await Api.update_cybersec_plan_doc({ id: row.id, file_no: nextFileNo });
            if (res.code === Api.C_OK) {
    ...
    const columns: any[] = [
        ...
        { title: "文档版本", dataIndex: "version", width: "7%" },
        {
            ...
            render: (value: string, row: any) => {
                ...
                if (isEditing) {
                    return (
                        <Input autoFocus size="small" value={data.editingFileNoValue} disabled={isSaving}
                ...
                return <span className="risk-doc-file-no-cell" title="单击编辑文件编号" onClick={() => handleStartEditFileNo(row)}>{value || "-"}</span>;
            },
        },
        { title: "变更说明", dataIndex: "change_log", width: "10%" },
        { title: ts("create_time"), dataIndex: "create_time", width: "15%" },
        {
            title: ts("action"), width: "26%", className: "risk-doc-action-col",
            render: (_: any, row: any) => (
                <Space size={4} className="risk-doc-action-space">
                    <Button type="link" size="small" onClick={() => navigate(`/cybersec_plan_docs/view/${row.id}`)}>{ts("view")}</Button>
                    <Button type="link" size="small" onClick={() => navigate(`/cybersec_plan_docs/edit/${row.id}`)}>{ts("edit")}</Button>
                    <Button type="link" size="small" onClick={() => doDuplicate(row)}>复制</Button>
                    <Button type="link" size="small" loading={data.exportingId === row.id} onClick={() => doExport(row)}>导出</Button>
    ...
    return (
        <div className="div-v page">
                    ...
                    <Row gutter={20}>
                        <Col>
                            <Form.Item label={ts("srs_doc.select_product")} name="product_id">
                                <ProductVersionSelect products={data.products} allowClear includeAll
                                    namePlaceholder={ts("product.name")} versionPlaceholder={ts("product.full_version")}
            ...
            />
            <Modal width={620} centered title="新增网络安全风险管理计划" open={data.dlgType === DlgTypes.add}
                ...
                <Form form={addForm} layout="vertical">
                    ...
                    <Form.Item label="变更说明" name="change_log"><Input.TextArea rows={3} allowClear /></Form.Item>
                </Form>
            </Modal>
            <Modal centered title={ts("action")} open={data.dlgType === DlgTypes.delete} maskClosable={false}
                confirmLoading={data.loading} onOk={doDelete} onCancel={() => dispatch({ dlgType: null })}>
                {ts("confirm_delete")}
            </Modal>
            <Modal centered width={520} title="复制" open={data.dlgType === DlgTypes.copy} maskClosable={false}
                confirmLoading={data.loading} onOk={doCopy} onCancel={() => dispatch({ dlgType: null })}>
                <div style={{ lineHeight: 1.8 }}>
                    <div style={{ marginBottom: 12 }}>复制到目标产品（默认当前产品，可选其它产品）：</div>
                    <ProductVersionSelect products={data.products} value={data.copyProductId}
                        namePlaceholder={ts("product.name")} versionPlaceholder={ts("product.full_version")}
                        onChange={(value: any) => dispatch({ copyProductId: value })}
                    />
                    <div style={{ color: "#888", marginTop: 12 }}>版本号自动生成：同产品在原版本号上递增；跨产品按目标产品现有最大版本递增。</div>
                </div>
            </Modal>
        </div>
    );

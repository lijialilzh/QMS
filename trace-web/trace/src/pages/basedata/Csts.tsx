...
const DetailDlg = ({ data, dispatch, onSaved }: any) => {
    ...
    const doEdit = () => {
        editForm.validateFields().then((values) => {
            ...
            const fn_request = data.dlgType === DlgTypes.edit ? Api.update_cst : Api.add_cst;
    ...
    useEffect(() => {
        if (data.dlgType === DlgTypes.add || data.dlgType === DlgTypes.edit) {
            ...
            if (data.dlgType === DlgTypes.edit) {
                dispatch({ loading: true });
                Api.get_cst({ id: data.targetRow.id }).then((res: any) => {
                    if (res.code === Api.C_OK) {
                ...
                <Form form={editForm} className="expand" onFinish={(_values) => {}}>
                    ...
                    <Row gutter={24}>
                        <Col span={24}>
                            <Form.Item
                                label={ts("cst.code")}
                                rules={[{ required: true, message: sprintf(ts("msg_input"), { label: ts("cst.code") }) }]}
                                name="code">
                                <Input allowClear />
                            </Form.Item>
                        </Col>
                    ...
                    <Row gutter={24}>
                        <Col span={24}>
                            <Form.Item label={ts("cst.category")} name="category">
                                <Select allowClear options={CATEGORIES.map((item: any) => ({ label: item, value: item }))} />
                            </Form.Item>
                        </Col>
                    </Row>
                    <Row gutter={24}>
                        <Col span={24}>
                            <Form.Item label={ts("cst.module")} name="module">
                                <Input allowClear />
                            </Form.Item>
                        </Col>
                    </Row>
                    <Row gutter={24}>
                        <Col span={24}>
                            <Form.Item label={ts("cst.connection")} name="connection">
                                <Input allowClear />
                            </Form.Item>
                        </Col>
                    </Row>
                    <Row gutter={24}>
                        <Col span={24}>
                            <Form.Item
                                label={ts("cst.description")}
                                rules={[{ required: true, message: sprintf(ts("msg_input"), { label: ts("cst.description") }) }]}
                                name="description">
                    ...
                    <Row gutter={24}>
                        <Col span={24}>
                            <Form.Item label={ts("cst.harm")} name="harm">
                                <Input.TextArea allowClear />
                            </Form.Item>
                        </Col>
                    </Row>
...
export default () => {
    ...
    const doSearch = (params: any, pageIndex: any, pageSize: any) => {
        dispatch({ loading: true });
        Api.list_cst({ ...params, page_index: pageIndex - 1, page_size: pageSize }).then((res: any) => {
            if (res.code === Api.C_OK) {
    ...
    const doDelete = () => {
        dispatch({ loading: true });
        Api.delete_cst({ id: data.targetRow.id }).then((res: any) => {
            if (res.code === Api.C_OK) {
                dispatch({ loading: false, dlgType: null });
                message.success(res.msg);
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                for (const id of keys) {
                    try {
                        const res: any = await Api.delete_cst({ id });
            ...
            <div className="div-h searchbar list-searchbar-align">
                ...
                <div className="div-h hspace">
                    <Upload
                        ...
                        accept=".xlsx"
                        beforeUpload={(file) => {
                            Api.import_csts({ file }).then((res: any) => {
                                if (res.code === Api.C_OK) {
                                    message.success(res.msg);
                    ...
                    <Button
                        ...
                        onClick={() => {
                            ...
                            Api.export_csts({ ...queryForm.getFieldsValue(), page_index: 0, page_size: 2000 }).then((res: any) => {

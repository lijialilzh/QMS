...
// 从时间线里找「软件配置管理计划」文件所在行的日期（取最早匹配行），格式「YYYY年M月D日」
const computeFileDate = (rows: any[], keyword = "软件配置管理计划"): string => {
    const num = (v: any) => parseInt(String(v ?? "").replace(/[^\d]/g, ""), 10);
...
export default () => {
        ...
        new Promise((resolve) => {
            ...
            Promise.all([
                ...
                out = fillMilestone(out, {
                    ...
                    planDate: latestDateFor(tlRows, ["软件配置管理计划"]),
    ...
    const load = () => {
        ...
        Api.get_scm_doc({ id }).then((res: any) => {
        ...
        Modal.confirm({
            ...
            onOk: () => {
                ...
                Api.rebind_product({ id, product_id: newId }).then((res: any) => {
                    ...
                    const doc = res.data || {};
                    const sections = ensureKeys((doc.content && doc.content.sections) || []);
                    // 后端已强制重填封面/修订/产品名；前端 autofill 再处理产品概况/周期等附加信息
    ...
    const doSave = () => {
        ...
        Api.update_scm_doc({ id, content, product_id: data.doc.product_id, version: data.doc.version }).then((res: any) => {
    ...
    const doExport = async () => {
        ...
        try {
            const res: any = await Api.export_scm_doc({ id });
        ...
        <div className="div-v page pdp-detail">
            <div className="div-h pdp-toolbar">
                ...
                <Space>
                    ...
                    <Button onClick={() => navigate("/scm_docs")}>{ts("back")}</Button>
                </Space>
            </div>

...
// 从时间线里找「用户测试计划」文件所在行的日期（取最早匹配行），格式「YYYY年M月D日」
const computeFileDate = (rows: any[], keyword = "用户测试计划"): string => {
    const num = (v: any) => parseInt(String(v ?? "").replace(/[^\d]/g, ""), 10);
...
export default () => {
        ...
        new Promise((resolve) => {
            ...
            Promise.all([
                ...
                out = fillMilestone(out, {
                    ...
                    planDate: latestDateFor(tlRows, ["用户测试计划"]),
    ...
    const load = () => {
        ...
        Api.get_utp_doc({ id }).then((res: any) => {
    ...
    const doSave = () => {
        ...
        Api.update_utp_doc({ id, content, product_id: data.doc.product_id, version: data.doc.version }).then((res: any) => {
    ...
    const doExport = async () => {
        ...
        try {
            const res: any = await Api.export_utp_doc({ id });
        ...
        <div className="div-v page pdp-detail">
            <div className="div-h pdp-toolbar">
                ...
                <Space>
                    ...
                    <Button onClick={() => navigate("/utp_docs")}>{ts("back")}</Button>
                </Space>
            </div>
                    ...
                    <div className="pdp-editor">
                        {!active ? (
                            ...
                            <>
                                ...
                                {(() => {
                                    const rtBodyLock = readonly || isRuntimeLockedBody(active);
                                                    ...
                                                    return (
                                                        <div className="pdp-table-block" key={si}>
                                                            <div className="pdp-table-bar">
                                                                <span className="pdp-label">表格 {ti + 1}{rtLock ? "（运行环境自动获取，只读）" : ""}</span>
                                                                {!rtLock && (
                                                                    <Space size={4}>
                                                            ...
                                                            <Input.TextArea
                                                                ...
                                                                value={seg.caption || (active.table_captions && active.table_captions[ti]) || ""}
                                                                disabled={rtLock}
                                    ...
                                    return (
                                        <>
                                            <div className="pdp-field">
                                                <div className="pdp-label">正文{rtBodyLock ? "（运行环境自动获取，只读）" : ""}</div>
                                                <Input.TextArea
                                                    autoSize={{ minRows: 3, maxRows: 20 }}
                                                    value={active.body ?? ""}
                                                    disabled={rtBodyLock}

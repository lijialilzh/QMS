    ...
    while not ok:
        try:
            ...
            with sql_ctx.db():
                ...
                sql_ctx.db.session.execute(expression.text("ALTER TABLE IF EXISTS product ADD COLUMN IF NOT EXISTS overall_desc TEXT"))
                sql_ctx.db.session.execute(expression.text("ALTER TABLE IF EXISTS product ADD COLUMN IF NOT EXISTS design_address VARCHAR(512)"))

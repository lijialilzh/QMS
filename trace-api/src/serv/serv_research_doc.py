...
class Server(object):
    ...
    def __to_obj(self, row: ResearchDoc, product: Product = None, with_autofill=True):
        ...
        if product:
            ...
            if not (obj.file_no or "").strip():
                resolved = serv_review_util.resolve_doc_file_no(product.id, obj.file_no, obj.version, "research")
                if resolved:
    ...
    async def add_research_doc(self, form: ResearchDocForm):
        try:
            ...
            row.id = None
            row.file_no = serv_review_util.resolve_doc_file_no(form.product_id, form.file_no, form.version, "research") or None

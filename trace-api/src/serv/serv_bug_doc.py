...
class Server(object):
    ...
    def __cover_values(self, row):
        """按测试文件规则计算封面要回填的单元格值（姓名、日期、修订记录）。纯 DB 读取，不做文件 IO。"""
        ...
        vals = {
            "E7": drafter, "G7": rev_date,          # 编制人 / 日期
            "E8": reviewer, "G8": rev_date,         # 审核人 / 日期
            "E9": approver, "G9": rev_date,         # 批准人 / 日期
            "E10": rev_date,                        # 生效日期
    ...
    def __ensure_filled(self, src_path, values):
        """返回封面已回填的 xlsx 磁盘路径（带缓存）。cover 值不变则复用缓存，不再重复生成。
        纯文件/CPU 操作，可放线程池执行；出错回退原文件路径。"""
        import hashlib
        import json
        key = hashlib.md5(json.dumps(values, ensure_ascii=False, sort_keys=True).encode("utf-8")).hexdigest()
        ...
        try:
            if os.path.exists(filled) and os.path.exists(keyf):
                with open(keyf, "r", encoding="utf-8") as f:
                    if f.read().strip() == key:
                        return filled
            data = self.__build_filled(src_path, values)
    ...
    def __build_filled(self, src_path, values):
        """在 zip 层复制整个 xlsx，仅改「封面」页的目标单元格；纯文件操作，可放线程池执行。"""
        with zipfile.ZipFile(src_path, "r") as zin:
            ...
            rels = zin.read("xl/_rels/workbook.xml.rels").decode("utf-8", errors="ignore")
            mt = re.search(r'<Relationship Id="' + re.escape(rid) + r'"[^>]*Target="([^"]+)"', rels)
    ...
    async def download_bug_doc(self, id: int):
        ...
        try:
            values = self.__cover_values(row)  # 纯 DB 读取
            filled = await run_in_threadpool(self.__ensure_filled, path, values)  # 生成/取缓存，放线程池
    ...
    async def preview_bug_doc(self, id: int):
        ...
        if not path:
            return Resp.resp_err(msg=ts("msg_obj_null"))
        try:
            cover_vals = self.__cover_values(row)
        except Exception:

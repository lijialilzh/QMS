...
class Server(object):
    ...
    async def rebind_product(self, id: int, product_id: int):
        ...
        try:
            row: StpDoc = db.session.execute(select(StpDoc).where(StpDoc.id == id)).scalars().first()
            if not row:
                return Resp.resp_err(msg=ts("msg_obj_null"))
            ...
            if not product:
                return Resp.resp_err(msg=ts("msg_obj_null"))
            db.session.execute(delete(StpDoc).where(StpDoc.product_id == product_id, StpDoc.version == row.version, StpDoc.id != id))

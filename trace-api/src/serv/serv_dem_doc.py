...
# 周检查表：日期行按产品时间线「开始开发~测试结束」每周一条(周一~周五)自动生成；每格 是/否 复选框。
...
_ASSETS = [
    ["SER01405", _UBUNTU_INFO],
    ["SER01358", _UBUNTU_INFO],
    ["SER01268", _UBUNTU_INFO],
    ["SER01100", _UBUNTU_INFO],
    ["SER01458", _UBUNTU_INFO],
    ["ISER21120027", _MAC_INFO],
    ["ISER21120029", _MAC_INFO],
    ["ISER21120023", _MAC_INFO],
]

# 各资产的周检查表（照文件：SER01405=服务器，其余(不含SER01458)=开发机）。
_ASSET_CHECKS = [
    ("SER01405", "server"),
    ("SER01358", "dev"),
    ("SER01268", "dev"),
...
def _default_content():
    ...
    return {"desc": _DESC, "assets": assets, "checks": checks}
...
class Server(object):
    ...
    def __dev_test_range(self, prod_id):
        ...
        start_d = min(dev_dates) if dev_dates else None
    ...
    def __autofill(self, content, prod_id, product=None, force=False):
        ...
        if prod_id:
            ...
            tpm = next((m.name for m in members if "TPM" in str(m.role or "")), "")
            tpm = (tpm or "").strip()
            if tpm:
                checker_val = serv_review_util._sign_by_name(tpm) or tpm
    ...
    def __dhf_file_no(self, prod_id):
        """从产品 DHF 按名称「开发环境维护说明」精确匹配取文件编号（避免误配「模型开发环境维护说明」）。"""
        if not prod_id:
            return ""
        row = db.session.execute(
            select(ProdDhf).where(ProdDhf.prod_id == prod_id, ProdDhf.name == "开发环境维护说明")
        ...
        if not row:
            row = db.session.execute(
                select(ProdDhf).where(
                    ...
                    ProdDhf.name.like("%开发环境维护说明%"),
                    ~ProdDhf.name.like("模型%"),
                ).order_by(ProdDhf.id.asc())
            ).scalars().first()
        return (row.code or "").strip() if row and row.code else ""

    def __to_obj(self, row: DemDoc, product: Product = None):
        obj = DemDocObj(**row.dict())
    ...
    async def add_dem_doc(self, form: DemDocForm):
        try:
            sql = select(func.count(DemDoc.id)).where(
                DemDoc.product_id == form.product_id,
                DemDoc.version == form.version,
            )
            if db.session.execute(sql).scalar() > 0:
                return Resp.resp_err(msg=ts("msg_obj_exist"))
            row = DemDoc(**form.dict(exclude_none=True))
            row.id = None
            row.content = self.__normalize_content(row.content)
            db.session.add(row)
            db.session.commit()
            return Resp.resp_ok(data=DemDocForm(id=row.id))
        except Exception:
            logger.exception("")
            db.session.rollback()
        return Resp.resp_err(msg=ts(msg_err_db))

    async def duplicate_dem_doc(self, id: int, product_id: int = None):
        try:
            fromdoc: DemDoc = db.session.execute(select(DemDoc).where(DemDoc.id == id)).scalars().first()
            if not fromdoc:
                return Resp.resp_err(msg=ts("msg_obj_null"))
            target_pid = product_id or fromdoc.product_id
            all_versions = db.session.execute(select(DemDoc.version).where(DemDoc.product_id == target_pid)).scalars().all()
    ...
    async def update_dem_doc(self, form: DemDocForm):
        try:
            row: DemDoc = db.session.execute(select(DemDoc).where(DemDoc.id == form.id)).scalars().first()
            if not row:
                return Resp.resp_err(msg=ts("msg_obj_null"))
    ...
    async def delete_dem_doc(self, id: int):
        db.session.execute(delete(DemDoc).where(DemDoc.id == id))
        db.session.commit()
        return Resp.resp_ok()

    async def get_dem_doc(self, id: int):
        sql = select(DemDoc, Product).join(Product, DemDoc.product_id == Product.id).where(DemDoc.id == id)
    ...
    async def list_dem_doc(self, op_user: UserObj = None, product_id: int = 0, version: str = None, page_index: int = 0, page_size: int = 10):
        wheres = []
        if product_id:
            wheres.append(DemDoc.product_id == product_id)
        if version:
            wheres.append(DemDoc.version.like(f"%{version}%"))
        if op_user and op_user.id != 1 and op_user.role_code == Roles.product_manager.value.code:
            wheres.append(Product.create_user_id == op_user.id)
        sql_total = select(func.count(DemDoc.id)).join(Product, DemDoc.product_id == Product.id).where(*wheres)
        total = db.session.execute(sql_total).scalar() or 0
        sql = (
            select(DemDoc, Product)
            .join(Product, DemDoc.product_id == Product.id)
            .where(*wheres)
            .order_by(DemDoc.id.desc())
            .offset(page_index * page_size)
            .limit(page_size)
        )
        rows: List[DemDocObj] = [self.__to_obj(doc, product) for doc, product in db.session.execute(sql).all()]
        return Resp.resp_ok(data=Page(total=total, rows=rows, page_index=page_index, page_size=page_size))

    # ---------------- 导出 Word ----------------
    async def export_dem_doc(self, output, id: int):
        resp = await self.get_dem_doc(id)
        obj: DemDocObj = resp.data
        if obj is None:
        ...
        def set_cell(cell, text, bold=False, align=WD_ALIGN_PARAGRAPH.LEFT, size=10.0):
            ...
            if s.startswith("data:image"):
                try:
                    ...
                    para.add_run().add_picture(BytesIO(base64.b64decode(b64)), height=Pt(26))
        ...
        def set_yesno(cell, mark):
            # 单元格内两个复选框，换行显示：☑是 / ☐否（选中☑加文本呈现选择符，单色白底）
            cell.text = ""
        ...
        docx_util.fonted_txt(title, "开发环境维护说明", font_size=18.0, bold=True)
        ...
        for chk in (c.get("checks") or []):
            ...
            for row in (chk.get("rows") or []):
                ...
                for idx, col in enumerate(leaves):
                    ...
                    if t == "date":
                        ...
                        set_cell(cells[idx], row.get("checker", ""), align=WD_ALIGN_PARAGRAPH.CENTER, size=9.0)
            # 固定列宽：日期/问题/检查人 更宽，检查项较窄
            tb.autofit = False
            ...
            if _grid is not None:
                for _gc in list(_grid):
                    _grid.remove(_gc)
                for _col in leaves:
                    _w = 1300 if _col["type"] == "date" else 2200 if _col["type"] == "problem" else 1500 if _col["type"] == "checker" else 720

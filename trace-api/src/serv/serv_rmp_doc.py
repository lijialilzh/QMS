...
class Server(object):
    ...
    def __to_obj(self, row: RmpDoc, product: Product = None, with_autofill=True):
        ...
        if product:
            ...
            if not (obj.file_no or "").strip():
                resolved = serv_review_util.resolve_doc_file_no(product.id, obj.file_no, obj.version, "rmp")
                if resolved:
    ...
    async def add_rmp_doc(self, form: RmpDocForm):
        try:
            ...
            row.id = None
            row.file_no = serv_review_util.resolve_doc_file_no(form.product_id, form.file_no, form.version, "rmp") or None

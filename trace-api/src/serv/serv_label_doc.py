...
class Server(object):
    ...
    def __to_obj(self, row: LabelDoc, product: Product = None):
        ...
        if product:
            ...
            if not (obj.file_no or "").strip():
                resolved = serv_review_util.resolve_doc_file_no(product.id, obj.file_no, obj.version, "label")
                if resolved:
    ...
    async def add_label_doc(self, form: LabelDocForm):
        try:
            ...
            row.id = None
            row.file_no = serv_review_util.resolve_doc_file_no(form.product_id, form.file_no, form.version, "label") or None
        ...
        try:
            ...
            newdoc = LabelDoc(
                ...
                file_no=sync_file_no_version(
                    ...
                    or serv_review_util.resolve_doc_file_no(target_pid, "", version, "label")
                    or "",
                    version,
                ) or None,

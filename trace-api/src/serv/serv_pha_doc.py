...
class Server(object):
    ...
    def __to_obj(self, row: PhaDoc, product: Product = None):
        ...
        if product:
            ...
            if not (obj.file_no or "").strip():
                resolved = serv_review_util.resolve_doc_file_no(product.id, obj.file_no, obj.version, "pha")
                if resolved:
    ...
    async def add_pha_doc(self, form: PhaDocForm):
        try:
            ...
            row.id = None
            row.file_no = serv_review_util.resolve_doc_file_no(form.product_id, form.file_no, form.version, "pha") or None

...
# 自研软件网络安全研究报告（独立文档模块）数据模型。
# 单表 nsr_doc：整份文档（封面/各章节文本/表格/图片/自动获取标记）以 content(JSON) 存储。
# 自动获取章节通过 content 节点的 ref_type 标记，在 get/export 时由服务层注入产品数据，不落库覆盖手工章节。
